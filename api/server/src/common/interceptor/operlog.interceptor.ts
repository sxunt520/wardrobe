  ...
  intercept(context: ExecutionContext, next: CallHandler): Observable<any> {
    ...
    return next
      ...
      .pipe(
        map((resultData) => {
          ...
          if (!resultData?.code || resultData.code === 200) {
            void this.logService.logAction({ costTime, resultData, handlerName, title: summary, businessType: logConfig?.businessType });
          } else {
            void this.logService.logAction({ costTime, errorMsg: resultData.msg, handlerName, title: summary, businessType: logConfig?.businessType });
          }
          return resultData;
        }),
      )
      .pipe(
        catchError((err) => {
          const costTime = Date.now() - now;
          void this.logService.logAction({ costTime, errorMsg: err.response, handlerName, title: summary, businessType: logConfig?.businessType });
