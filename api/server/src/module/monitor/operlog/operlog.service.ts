...
export class OperlogService {
  ...
  async logAction({
    ...
    const user = this.request.user?.user || this.request.user || {};
    let operLocation = '';
    try {
      operLocation = await this.axiosService.getIpAddress(ip);
    } catch {}
    const safeParams = this.sanitize({ ...body, ...query });

    const params = {
      title,
      method: handlerName,
      operName: user.nickName || user.userName || '',
      ...
      operParam: JSON.stringify(safeParams),
  ...
  private sanitize(value: Record<string, any>) {
    const hiddenKeys = new Set(['password', 'oldPassword', 'newPassword', 'secret', 'token', 'authorization']);
    return Object.fromEntries(Object.entries(value || {}).map(([key, item]) => [key, hiddenKeys.has(key) ? '******' : item]));
  }
